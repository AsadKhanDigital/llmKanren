...
def parse_log_file(log_file_path):
    ...
    for i in range(1, len(test_cases), 2):
        ...
        if llm_2_match:
            ...
            num_candidates_match = re.search(r"----------------------------------------\nLLM \(n=2\)\n----------------------------------------\nnum_candidates: (\d+)", 
                                           test_content, re.DOTALL)
            num_candidates = num_candidates_match.group(1) if num_candidates_match else "N/A"
...
def format_number(number_str):
    """Format a number string with comma separators for better readability"""
    try:
        # For integers
    ...
    with open(output_file, 'w', newline='') as f:
        ...
        for result in results:
            ...
            if "expert" in result:
                ...
                row.append(f"inc_count: {format_number(expert_data['inc_count'])}\nevalo_count: {format_number(expert_data['evalo_count'])}\nmk_time: {format_number(expert_data['mk_time'])}s")
            else:
                row.append("N/A")
            ...
            if "zinkov_2" in result:
                ...
                row.append(f"inc_count: {format_number(zinkov_2_data['inc_count'])}\nevalo_count: {format_number(zinkov_2_data['evalo_count'])}\nmk_time: {format_number(zinkov_2_data['mk_time'])}s")
            else:
                row.append("N/A")
            ...
            if "zinkov_3" in result:
                ...
                row.append(f"inc_count: {format_number(zinkov_3_data['inc_count'])}\nevalo_count: {format_number(zinkov_3_data['evalo_count'])}\nmk_time: {format_number(zinkov_3_data['mk_time'])}s")
            else:
                row.append("N/A")
            ...
            if "llm_2" in result:
                ...
                row.append(f"inc_count: {format_number(llm_2_data['inc_count'])}\nevalo_count: {format_number(llm_2_data['evalo_count'])}\nllm_time: {format_number(llm_2_data['llm_time'])}s\nmk_time: {format_number(llm_2_data['mk_time'])}s\nnum_candidates: {format_number(llm_2_data['num_candidates'])}")
            else:
                row.append("N/A")
            ...
            if "llm_3" in result:
                ...
                row.append(f"inc_count: {format_number(llm_3_data['inc_count'])}\nevalo_count: {format_number(llm_3_data['evalo_count'])}\nllm_time: {format_number(llm_3_data['llm_time'])}s\nmk_time: {format_number(llm_3_data['mk_time'])}s\nnum_candidates: {format_number(llm_3_data['num_candidates'])}")
            else:
                row.append("N/A")
            ...
            if "llm_4" in result:
                ...
                row.append(f"inc_count: {format_number(llm_4_data['inc_count'])}\nevalo_count: {format_number(llm_4_data['evalo_count'])}\nllm_time: {format_number(llm_4_data['llm_time'])}s\nmk_time: {format_number(llm_4_data['mk_time'])}s\nnum_candidates: {format_number(llm_4_data['num_candidates'])}")
            else:
                row.append("N/A")
